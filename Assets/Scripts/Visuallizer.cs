...
public class Visuallizer : MonoBehaviour
{
	...
	[SerializeField] Skelleton skeleton;
	void LateUpdate()
	{
		...
		var landMarkBuffer = holisticPipeline.poseLandmarkWorldBuffer;
		var faceLandMark = holisticPipeline.faceVertexBuffer;

		//holisticPipeline.
		var count = landMarkBuffer.count;
		Vector4[] posList = new Vector4[count];
		landMarkBuffer.GetData(posList);

		count = faceLandMark.count;
		Vector4[] faceList = new Vector4[count];
		faceLandMark.GetData(faceList);

		if (skeleton != null)
			skeleton.UpdatePos(posList, faceList);
	}
